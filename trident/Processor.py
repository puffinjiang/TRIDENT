...
class Processor:
    def __init__(
        ...
        if not (sys.version_info.major >= 3 and sys.version_info.minor >= 9):
            raise EnvironmentError(
                "Trident requires Python 3.9 or above. Python 3.10 is recommended."
            )

        self.job_dir = job_dir
        self.wsi_source = wsi_source
        self.wsi_ext = wsi_ext or (
            list(PIL_EXTENSIONS) + list(OPENSLIDE_EXTENSIONS) + list(SDPC_EXTENSIONS)
        )
        ...
        if custom_list_of_wsis is not None:
            ...
            valid_mpps = (
                wsi_df["mpp"].dropna().tolist() if "mpp" in wsi_df.columns else None
            )
        else:
            valid_mpps = None
        ...
        for wsi_idx, abs_path in enumerate(full_paths):
            ...
            tissue_seg_path = os.path.join(
                self.job_dir, "contours_geojson", f"{os.path.splitext(name)[0]}.geojson"
            )
    ...
    def run_segmentation_job(
        ...
        for wsi in self.loop:
            ...
            if os.path.exists(
                os.path.join(saveto, f"{wsi.name}.jpg")
            ) and not is_locked(os.path.join(saveto, f"{wsi.name}.jpg")):
    ...
    def run_patching_job(
        ...
        self.loop = tqdm(
            self.wsis,
            desc=f"Saving tissue coordinates to {saveto}",
            total=len(self.wsis),
        )
        for wsi in self.loop:
            # Check if patch coords already exist
            if os.path.exists(
                os.path.join(self.job_dir, saveto, "patches", f"{wsi.name}_patches.h5")
            ):
                self.loop.set_postfix_str(
                    f"Patch coords already generated for {wsi.name}. Skipping..."
                )
                update_log(
            ...
            if gdf.empty:
                self.loop.set_postfix_str(
                    f"Empty GeoDataFrame for {wsi.name}. Skipping..."
                )
                update_log(
                    os.path.join(self.job_dir, saveto, "_logs_coords.txt"),
                    f"{wsi.name}{wsi.ext}",
            ...
            try:
                self.loop.set_postfix_str(
                    f"Generating patch coords for {wsi.name}{wsi.ext}"
                )
                update_log(
                    os.path.join(self.job_dir, saveto, "_logs_coords.txt"),
                    f"{wsi.name}{wsi.ext}",
                    "LOCKED. Generating coords...",
                )
                create_lock(
                    os.path.join(
                        self.job_dir, saveto, "patches", f"{wsi.name}_patches.h5"
                    )
                )
    ...
    def run_slide_feature_extraction_job(
        ...
        if slide_encoder.enc_name.startswith("mean-"):
            slide_to_patch_encoder_name[slide_encoder.enc_name] = (
                slide_encoder.enc_name.split("mean-")[1]
            )  # e.g. mean-resnet18 -> resnet18

        # Setting I/O
        mustbe_patch_encoder = slide_to_patch_encoder_name[slide_encoder.enc_name]
        patch_features_dir = os.path.join(
            coords_dir, f"features_{mustbe_patch_encoder}"
        )
        if saveto is None:
            saveto = os.path.join(
                coords_dir, f"slide_features_{slide_encoder.enc_name}"
            )
        os.makedirs(os.path.join(self.job_dir, saveto), exist_ok=True)
        ...
        self.save_config(
            saveto=os.path.join(
                self.job_dir,
                coords_dir,
                f"_config_slide_features_{slide_encoder.enc_name}.json",
            ),
            local_attrs=local_attrs,
        ...
        self.loop = tqdm(
            self.wsis,
            desc=f"Extracting slide features using {slide_encoder.enc_name}",
            total=len(self.wsis),
        )
        for wsi in self.loop:
            # Check if slide features already exist
            slide_feature_path = os.path.join(
                self.job_dir, saveto, f"{wsi.name}.{saveas}"
            )
            if os.path.exists(slide_feature_path) and not is_locked(slide_feature_path):
                self.loop.set_postfix_str(
                    f"Slide features already extracted for {wsi.name}. Skipping..."
                )
                update_log(
                    os.path.join(
                        self.job_dir,
                        coords_dir,
                        f"_logs_slide_features_{slide_encoder.enc_name}.txt",
                    ),
                    f"{wsi.name}{wsi.ext}",
            ...
            patch_features_path = os.path.join(
                self.job_dir, patch_features_dir, f"{wsi.name}.h5"
            )
            if not os.path.exists(patch_features_path):
                self.loop.set_postfix_str(
                    f"Patch features not found for {wsi.name}. Skipping..."
                )
                update_log(
                    os.path.join(
                        self.job_dir,
            ...
            try:
                self.loop.set_postfix_str(
                    f"Extracting slide features for {wsi.name}{wsi.ext}"
                )
                ...
                update_log(
                    ...
                    f"{wsi.name}{wsi.ext}",
                    "LOCKED. Extracting slide features...",
                )
